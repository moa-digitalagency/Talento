...
- PP: Code pays d'origine ISO-2 (2 lettres)
- VVV: 3 premières lettres de la ville de résidence (en majuscules)
- NNNN: Numéro séquentiel (4 chiffres) - incrémenté par PAYS D'ORIGINE
- G: Genre (M/F)

Exemple: MACAS0001F (Origine Maroc, résidence Casablanca, 1ère personne originaire du Maroc, Femme)

Important: Le compteur est global par pays d'origine, donc:
- MACAS0001F = 1ère personne originaire du Maroc (résidant à Casablanca)
- MARAB0002M = 2ème personne originaire du Maroc (résidant à Rabat)
- SNDAG0001F = 1ère personne originaire du Sénégal (résidant à Dakar)
"""
from app import db
...
def generate_cinema_unique_code(country_of_origin_name, city_of_residence_name, gender):
    """
    Générer un code unique pour un talent CINEMA
    
    Args:
        country_of_origin_name (str): Nom du pays d'origine
        city_of_residence_name (str): Nom de la ville de résidence
    ...
    Format:
        - PP: Code pays d'origine (2 lettres, ex: MA pour Maroc)
        - VVV: Code ville de résidence (3 lettres, ex: CAS pour Casablanca)
        - NNNN: Numéro séquentiel (4 chiffres, incrémenté par PAYS D'ORIGINE)
        - G: Genre (M/F)
    
    Exemple: MACAS0001F (Maroc pays d'origine, résidence à Casablanca, 1ère personne du Maroc, Femme)
    
    Note:
        Le numéro séquentiel est incrémenté par PAYS D'ORIGINE uniquement, 
    ...
    """
    # Obtenir le code pays d'origine (2 lettres)
    country_code = get_country_code(country_of_origin_name)
    
    # Obtenir le code ville de résidence (3 lettres)
    city_code = clean_city_code(city_of_residence_name)
